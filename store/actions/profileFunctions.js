...
import { increaseFollowing, decreaseFollowing, setUserProfile } from './user'
...
export const FetchProfile = (userProfile) => {
    return async (dispatch, getState) => {
        try {
            ...
            const response = await axios.get(`http://127.0.0.1:8000/userprofile/${userProfile}/`)
...
export const FetchUserProfile = (userDetail) => {
    return async (dispatch, getState) => {
        try {
            ...
            const response = await axios.get(`http://127.0.0.1:8000/userprofile/${userDetail}/`)
            if (response.status !== 200){
                throw new Error('Something went wrong!')
            }
...
export const FollowProfile = (username) => {
    return async (dispatch, getState) => {
        try {
            ...
            const response = await axios.get(`http://127.0.0.1:8000/userprofile/${username}/?action=${is_following ? 'unfollow' : 'follow'}`)
            if (response.status !== 200){
                throw new Error('Something went wrong!')
            }
...
export const UpdateProfile = (image, body, navigation) => {
    return async (dispatch, getState) => {
        try {
            ...
            xhr.open("PATCH", `http://127.0.0.1:8000/userprofile/${username}/`, true);
